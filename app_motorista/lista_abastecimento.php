...
$page_title = "Lista de Abastecimentos"; 
...
// Adiciona filtros de data (no formato YYYY-MM-DD, que o input type="date" envia)
if (!empty($data_inicial)) {
    $where_clauses[] = "data_abastecimento >= ?";
    $params_types .= "s";
...
// NOVIDADE: Adicionando foto1_path e foto2_path ao SELECT
$sql = "SELECT id, valor, litros, hodometro, media, data_abastecimento, vinculado_viagem_id, foto1_path, foto2_path 
        FROM abastecimentos 
        {$where_sql}
...
try {
    ...
    $erro_db = "Erro inesperado ao carregar abastecimentos: " . $e->getMessage();
}
// -----------------------------------------------------------------
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .data-table { width: 100%; border-collapse: collapse; font-size: 0.8em; margin-bottom: 20px; }
        .data-table th, .data-table td { padding: 8px 3px; text-align: center; border-bottom: 1px solid #ddd; }
        ...
        /* Estilo para ÍCONE de foto */
        .icon-photo { color: #28a745; font-size: 1.2em; } /* Verde para foto */
        .icon-no-photo { color: #ccc; font-size: 1.2em; } /* Cinza claro para sem foto */
...
<div class="main-content">
    ...
    <form method="GET" action="lista_abastecimento.php" class="filter-form">
        ...
        <button type="button" onclick="window.location.href='lista_abastecimento.php'" title="Limpar Filtro"><i class="fas fa-undo"></i></button>
    </form>
    
    <div class="list-card">
        <h2 class="list-title">Lista de Abastecimentos</h2>
        ...
        <?php elseif (empty($abastecimentos)): ?>
            <p>Nenhum abastecimento encontrado para este usuário.</p>
        <?php else: ?>
            <table class="data-table">
                <thead>
                    <tr>
                        <th>DATA</th> <th>VALOR</th>
                        <th>LITROS</th>
                        <th>HODÔMETRO</th>
                        <th>MÉDIA</th>
                        <th>VIAGEM</th>
                ...
                <tbody>
                    ...
                    foreach ($abastecimentos as $abastecimento): 
                        ...
                        // Lógica para verificar se há fotos
                        $tem_foto = !empty($abastecimento['foto1_path']) || !empty($abastecimento['foto2_path']);
                    ?>
                    <tr onclick="window.location.href='visualizar_abastecimento.php?id=<?php echo $abastecimento['id']; ?>'">
                        <td><?php echo date('d/m/Y', strtotime($abastecimento['data_abastecimento'])); ?></td>
                        <td>R$ <?php echo number_format($abastecimento['valor'], 2, ',', '.'); ?></td>
                        <td><?php echo number_format($abastecimento['litros'], 2, ',', '.'); ?>L</td>
                        <td><?php echo number_format($abastecimento['hodometro'], 0, ',', '.'); ?>KM</td>
                        <td><?php echo number_format($abastecimento['media'], 2, ',', '.'); ?></td>
                        <td><?php echo $abastecimento['vinculado_viagem_id'] ?: '-'; ?></td>
                        <td>
                            <?php if ($tem_foto): ?>
                                <i class="fas fa-camera icon-photo" title="Comprovante enviado"></i>
                            <?php else: ?>
                                <i class="fas fa-minus icon-no-photo" title="Sem comprovante"></i>
                            <?php endif; ?>
                        </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <div class="table-footer">
                <span>Total Gasto: R$ <?php echo number_format($total_valor, 2, ',', '.'); ?></span>
                <span>Total Litros: <?php echo number_format($total_litros, 2, ',', '.'); ?>L</span>
            </div>
            
        <?php endif; ?>
        
        <a href="abastecimento.php" class="btn-voltar">VOLTAR</a>
