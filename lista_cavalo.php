...
// --------------------------------------------------------------
// HANDLER: DELETE via AJAX - lista_cavalo.php?action=delete
// --------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'delete') {
    header('Content-Type: application/json');

    $data = json_decode(file_get_contents("php://input"), true);
    $cavalo_id = isset($data['id']) ? $data['id'] : null;

    if (empty($cavalo_id) || !is_numeric($cavalo_id)) {
        return_json(['success' => false, 'message' => 'ID de Cavalo inválido ou ausente.'], 400);
    }

    $sql_delete = "DELETE FROM caminhoes_cavalo WHERE id = ?";

    if ($stmt = $conn->prepare($sql_delete)) {
        $stmt->bind_param("i", $cavalo_id);

        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                return_json(['success' => true, 'message' => 'Cavalo deletado com sucesso!']);
            } else {
                return_json(['success' => false, 'message' => 'Cavalo não encontrado ou já deletado.'], 404);
            }
        } else {
...
// --------------------------------------------------------------
// HANDLER: EDIT via AJAX - lista_cavalo.php?action=edit
// --------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'edit') {
    ...
    if (empty($cavalo_id) || !is_numeric($cavalo_id) || empty($dados)) {
        return_json(['success' => false, 'message' => 'Dados de Cavalo ou ID inválidos.'], 400);
    }
    ...
    $tipos .= 'i';
    $valores[] = (int)$cavalo_id;

    $sql_update = "UPDATE caminhoes_cavalo SET " . implode(', ', $campos_para_update) . " WHERE id = ?";
    ...
    if ($stmt = $conn->prepare($sql_update)) {
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                return_json(['success' => true, 'message' => 'Cavalo atualizado com sucesso!']);
            } else {
...
// --------------------------------------------------------------
$sql = "SELECT id, modelo, placa, renavam, ano, observacao FROM caminhoes_cavalo ORDER BY id ASC";
$result_cavalo = $conn->query($sql);
...
<head>
    ...
    <style>
        ...
        /* Estilo para o novo botão 'CARRETAS' */
        .btn-carretas { background-color: #3498db !important; } .btn-carretas:hover { background-color: #2980b9 !important; }
        ...
        /* Ajuste do card para ter padding e sombra */
        .card { 
            padding: 20px; 
...
<div class="container-geral">
    ...
    <div class="conteudo-principal">
        <div class="caminho-navegacao">Home > Cadastro de Cavalo > Lista de Cavalos</div>
        ...
        <div class="card"> 
            ...
            <div class="listagem-container">
                <table class="tabela-lista">
                    ...
                    <tbody>
                        <?php
                        if ($result_cavalo->num_rows > 0) {
                            while($row = $result_cavalo->fetch_assoc()) {
                            ...
                            echo "<tr><td colspan='6' style='text-align: center;'>Nenhum cavalo (caminhão) cadastrado.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
            ...
            <div class="botoes-rodape">
                ...
                <button 
                    ...
                    class="btn-carretas"
                >
                    <i class="fas fa-trailer"></i> CARRETAS
                </button>
                <button type="button" onclick="window.location.href='cadastro_cavalo.php'">CADASTRAR</button>
                <button type="button" onclick="window.location.href='cadastro_cavalo.php'">VOLTAR</button>
            </div>
        </div> </div>
</div>
...
<script>
    ...
    btnSalvar.addEventListener('click', function() {
        ...
        const idCavalo = linhaSelecionada.getAttribute('data-id');
        const dadosEditados = {};
        ...
        fetch('lista_cavalo.php?action=edit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: idCavalo, dados: dadosEditados })
        })
        .then(response => {
            if (!response.ok) {
    ...
    // 5. Deletar cavalo (AJAX para o mesmo arquivo ?action=delete)
    btnDeletar.addEventListener('click', function() {
        if (!linhaSelecionada || modoEdicao) return;

        const idCavalo = linhaSelecionada.getAttribute('data-id');
        const modeloCavalo = linhaSelecionada.querySelector('[data-field="modelo"]').textContent.trim();

        if (!confirm(`Tem certeza que deseja DELETAR o Cavalo "${modeloCavalo}" (ID: ${idCavalo})? Essa ação é irreversível!`)) return;

        fetch('lista_cavalo.php?action=delete', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: idCavalo })
        })
        ...
        .catch(error => {
            ...
            alert('Ocorreu um erro inesperado ao deletar o Cavalo.');
        });
    });
</script>
